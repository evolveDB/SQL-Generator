...
def connect_server(dbname, server_name):
    if server_name == 'mysql':
        ...
        db = psycopg2.connect(database=dbname, user="postgres", password="postgres", host="166.111.121.62", port="5432")
        cursor = db.cursor()
        return db, cursor
    else:
        print('数据库连接不上...')
...
class GenSqlEnv(object):
    # optimization targets, involved schema
...
def test_generate(dbname, max_episodes):
    env = GenSqlEnv(100000, dbname, 0)
    episode = 0
    #max_episodes = 10000
    while episode < max_episodes:
        # print('第', episode, '条')
        current_state = env.reset() # [clause, space]
        ...
        if ep_steps <= SEQ_LENGTH or reward == env.bug_reward:
            ...
            episode += 1
            sql_path = root_path = os.path.abspath('.')
            sql = env.get_sql()
            with open(sql_path+'/sql_templates.log','a') as f:
                f.write(sql+'\n')
            print(sql)
            # print('reward:', reward)

'''
self.target_type: control the targets [0: cost, 1: cardinality]
server_name='postgresql'
state: action (selected token)
'''

if __name__ == '__main__':
    test_generate('tpch', 10000)
